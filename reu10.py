...
psi0[0] = np.exp(1j * 0.5 * np.pi * (1 - np.sign(x))) * np.tanh((x - x0) / np.sqrt(2))
psi0[1] = np.exp(-1j * 0.5 * np.pi * (1 - np.sign(x))) * np.tanh((x + x0) / np.sqrt(2))

for i in range(len(psi0)):
    psi0[i] /= np.sqrt(np.trapz(np.abs(psi0[i])**2, x))

# Split-Step and the Operators
def split_step(psi0, U, Omega, g, dt, steps, x):
    evo  = []
    psi = psi0.copy()
    ...
    for step in range(steps):
        ...
        expV1 = np.exp(-1j * V1 * dt / 2)
        expV2 = np.exp(-1j * V2 * dt / 2)
        ...
        expT1 = np.exp(-1j * T1 * dt)
        expT2 = np.exp(-1j * T2 * dt)
        psi[0] *= expV1
        psi[1] *= expV2
    ...
    evo = np.array(evo)           
    return psi[0], psi[1], evo

# Run simulation
psi1, psi2, evo = split_step(psi0, U, Omega, g, dt, steps, x)
...
plt.grid(True)
plt.legend()
plt.title("Final State Densities (Real Time Evolution)")
plt.show()
...
plt.suptitle("Probability Density of the Wavefunctions Over Time (Real Time Evolution)")
plt.tight_layout()
plt.show()
